...
def replacer(match):
    ...
    # Check if walls or layout changed (normalize space)
    old_norm = "".join(old_board.split())
    new_norm = "".join(new_board.replace("\\n", "\n").split())
    
    # Always update to be sure
    return f'{{ board: `{new_board.replace("\\n", "\n")}`, solutionUrl: \'{url}\', minMoves: {min_moves} }}'
...
print("All 60 puzzles evaluated and synchronized with solution hashes.")
